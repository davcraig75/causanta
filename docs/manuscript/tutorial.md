...
The binomial segregation is key: a cell with 20 ecDNA copies first replicates them during S phase (to ~40 copies), then partitions the replicated pool to each daughter via `Binomial(N', 0.5)`. Each daughter ends up with ~20 copies *on average* but with large random spread (the fraction of the replicated pool is centered on 0.5). This randomness is like nature running a controlled experiment! (See the Analysis Guide for why the daughter fraction is measured against the replicated pool, not the pre-replication count.)
...
> **Note on recovery.** β and δ act on continuous per-cell phenotypes and are recovered directly by 2SLS. α is recovered from inter-division intervals on the unconfounded (κ_hyp = 0) runs. γ is a *configured design parameter only*: baseline apoptosis is so low (5×10⁻⁵/hr) that essentially no cell dies, so there is no survival selection to detect and γ is not estimated. See the structural-parameter explorer (`docs/simulation_parameter_explorer.html`) to see each effect's response curve and identifiability.
...
When the simulator's structural outcome equation is inverted (so the LHS is `migration / (v_base · hypoxia_mult) − 1` and the RHS is `δ · EGFR`), both OLS and 2SLS recover δ = 0.050 ± 0.000 exactly, because once the hypoxia multiplier is undone the only remaining source of variation in the outcome is EGFR itself. The OLS-vs-IV bias only shows up on the *unstructured* linear regression of the raw phenotype on EGFR (the form you would write down without knowing the structural equation). On that scale the multi-seed pipeline (`output/multiseed_full_results.json`) reports, for the slope of VEGF on EGFR:

| Scenario | OLS slope | 2SLS slope | OLS bias |
|---|---|---|---|
| 2 mm baseline (κ_hyp = 1.5) | 4.24 ± 0.06 | 3.90 ± 0.06 | +8.7% ± 1.2% |
| 6 mm baseline (κ_hyp = 1.5) | 4.15 ± 0.10 | 3.70 ± 0.22 | +12.5% ± 4.1% |
| 2 mm reduced (κ_hyp = 0.5) | 5.58 ± 0.20 | 5.25 ± 0.25 | +6.3% ± 1.6% |
| 6 mm reduced (κ_hyp = 0.5) | 5.03 ± 0.10 | 4.96 ± 0.12 | +1.4% ± 0.6% |
| 2 mm removed (κ_hyp = 0.0) | 6.49 ± 0.24 | 6.55 ± 0.25 | −1.0% ± 0.4% |
| 6 mm removed (κ_hyp = 0.0) | 5.89 ± 0.25 | 5.94 ± 0.24 | −0.9% ± 0.6% |

The take-home: removing the hypoxia → EGFR edge (κ_hyp = 0) collapses OLS bias to within ~1% of IV, which is exactly what the structural causal model predicts when the confounding pathway is severed. Migration shows the same pattern at smaller magnitude.
