...
CTLs can kill multiple targets sequentially ("serial killing"). In generic immune
models, efficiency declines after ~10 kills (Boissonnas 2007); CAUSANTA's
GBM-immunosuppressed configuration models faster exhaustion, with the default
`max_kills_before_exhaustion = 3` (see Configuration Parameters below):
...
The **Default** values below are the GBM-immunosuppressed configuration shipped in
`causanta/simulate/params/default.json` and used by every canonical run
(`large_*`, `xlarge_*`, `multiseed_*`, `robustness_*`). These override the generic
fallback values hardcoded in the `ImmuneRecruitmentConfig` dataclass in `config.py`
(synapse 1.5 hr, kill 0.70, 10 kills), which are never used for any reported result.
...
## Observed Infiltration in Canonical Runs (v19)

Through v18 the immune module never actually fired in the production runs: the
`RecruitedImmune` (type 7) cell type was not defined in the production param
files, so `_step_immune_recruitment` returned immediately, and `Microglia`
(type 3) were likewise absent. As of v19 both types are defined and the tissue
is populated, so immune cells now appear in every canonical run:

- **Recruited immune infiltration is scale-dependent.** In the 2 mm baseline the
  tumor occupies a large fraction of the domain, VEGF near the perivascular
  niche exceeds `chemokine_threshold`, and ~390 immune cells are recruited by
  t = 240 hr. In the 6 mm baseline the tumor is small relative to the domain, so
  only single-digit numbers of immune cells are recruited by t = 300 hr —
  reproducing the immune-cold core characteristic of large GBM.
- **Resident microglia** are seeded at the configured fraction (≈ 90 cells in the
  2 mm baseline) and become reactive near the tumor.
- Because the GBM-immunosuppressed parameters are deliberately weak (kill
  probability 0.25, exhaustion after ~3 kills, recruitment 0.003/hr), immune
  pressure does not control the tumor — tumor populations grow through the
  entire simulation in all 30 multi-seed runs, as intended.
