...
To bound the seed-to-seed variability of these single-seed point estimates, we ran an additional reliability check at the 2000×2000 μm baseline scale across five independent random seeds. Mean PC discovery performance was F1 = 0.916 ± 0.085, recall = 1.000 ± 0.000, and precision = 0.853 ± 0.144. The IV estimate was substantially more stable than OLS across seeds (IV β = 4.02 ± 0.16, CV ≈ 4%; OLS β = 4.37 ± 0.43, CV ≈ 10%), and the first-stage F-statistic was always strong (range 106,000–230,000). The single-seed F1 = 0.727 reported above for this configuration falls at the low tail of the seed distribution; the central tendency is closer to F1 ≈ 0.92, confirming the constraint-based pipeline is reliable at this scale. Tumor population size was reproducible (12,173 ± 578 cells; CV ≈ 5%).
