...
This guide is written so that it can be read at two levels of detail. For a **biologist** who wants to understand what each analysis is doing and how to interpret the output, read §1 (Overview of Causal Inference — the conceptual frame), the introductory paragraph of §3 (the 2SLS recipe in plain English), §4 (Statistical Diagnostics — what each number means and what thresholds matter), and §8 (Common Pitfalls). For a **computational reader** who wants to reproduce the procedures, read §3 (the equations), §5 (Working with Lineage Data — the file formats and column conventions), §6 (Spatial Analysis), §7 (Time-Series Analysis), and §9 (Code Examples). The plain-language metrics primer in the main manuscript (Results, §How to read the statistics in this section) is intentionally shorter than §4 here — it explains *what each metric means*, while this guide also explains *how to compute and report it*.
...
The binomial segregation model predicts specific properties. **Important:** the daughter
fraction must be computed against the *replicated* pool (`parent_ecDNA_after + daughter_ecDNA`),
not the pre-replication parent count (`parent_ecDNA_before`). During S phase each copy replicates
(~95% fidelity), roughly doubling the pool to N' ≈ 2N before the binomial split, so
`daughter_ecDNA / parent_ecDNA_before` averages ~0.97 (≈ replication ratio / 2), whereas
`daughter_ecDNA / (parent_ecDNA_after + daughter_ecDNA)` averages 0.5 as expected.

| Statistic | Formula | Example (N=20, N' ≈ 40) |
|-----------|---------|----------------|
| Mean daughter fraction (of replicated pool) | 0.5 | 0.5 |
| Variance of fraction | p(1-p)/N' | ≈ 0.0063 |
| Standard deviation | sqrt(0.25/N') | ≈ 0.079 |

Canonical check (6 mm baseline, seed 43, t = 300 h, n = 19,534 divisions): mean fraction
0.4997, variance 0.0069, one-sample t-test versus 0.5 gives t = −0.52, p = 0.60.
...
# From lineage data: daughter fraction of the REPLICATED pool for each division.
# Use parent_ecDNA_after + daughter_ecDNA (the post-replication total), NOT
# parent_ecDNA_before (which predates S-phase replication and yields ~0.97).
daughter_fractions = [
    d['daughter_ecDNA'] / (d['parent_ecDNA_after'] + d['daughter_ecDNA'])
    for d in lineage
    if (d['parent_ecDNA_after'] + d['daughter_ecDNA']) > 0
]
...
| 100-200 um | 10-20 | Around hypoxia threshold |
| >200 um | <10 | Severely hypoxic |

The hypoxia threshold (`hypoxia_threshold_mmHg`) sets the binary `is_hypoxic` flag (HIF stabilization). It is **18 mmHg** in the canonical publication runs (`xlarge_*`, `large_*`, `multiseed_*` param files); the `config.py` dataclass fallback default is 10 mmHg. Below this O2 level, cells switch toward the "Go" phenotype (increased migration), upregulate VEGF, and the HIF-2α → EGFR coupling activates. The separate *proliferation* arrest threshold (`O2_prolif_threshold_mmHg`) is 8 mmHg for tumor cells, and the *necrosis* threshold is 2.5 mmHg.
...
def analyze_simulation(output_dir: Path):
    ...
    daughter_fractions = []
    for div in lineage:
        replicated_total = div['parent_ecDNA_after'] + div['daughter_ecDNA']
        if replicated_total > 0:
            frac = div['daughter_ecDNA'] / replicated_total
