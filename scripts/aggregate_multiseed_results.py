...
# Ground truth depends on the scenario: the is_hypoxic -> egfr edge only
# exists when the HIF coupling (kappa_hyp) is non-zero. The VEGF gating by
# hypoxia (is_hypoxic -> VEGF_secretion) is present in every scenario
# because tumor cells in the simulator only secrete VEGF when hypoxic,
# regardless of kappa_hyp.
GROUND_TRUTH_WITH_HIF = [
...
GROUND_TRUTH_REMOVED = [
    ("ecDNA_count", "egfr_expression"),
    # No is_hypoxic -> egfr_expression: kappa_hyp = 0
    ("egfr_expression", "VEGF_secretion"),
    ("is_hypoxic", "VEGF_secretion"),
    ("egfr_expression", "migration_rate"),
]
...
def analyze_run(run_dir: str, seed: int, scenario: str = "baseline") -> dict | None:
    ...
    gt = GROUND_TRUTH_REMOVED if scenario == "removed" else GROUND_TRUTH_WITH_HIF
...
def main() -> int:
    ...
    for (scale, scenario), runs in cells.items():
        ...
        for run_dir, seed in runs:
            r = analyze_run(run_dir, seed, scenario=scenario)
            if r is not None:
                per_seed.append(r)
