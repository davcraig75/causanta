...
class CellTypeConfig:
    ...
    # Optional spatially-varying migration effect (default OFF -> constant
    # ecDNA_effect_on_migration is used, preserving all existing runs). When
    # enabled, the per-copy migration coefficient delta becomes a step function
    # of the cell's distance from the tumor-seed centroid: cells inside
    # core_radius use delta_core, cells inside margin_radius use delta_margin,
    # and cells beyond margin_radius use delta_infiltrating. Used to benchmark
    # whether per-zone 2SLS recovers a KNOWN spatial gradient in the effect.
    ecDNA_migration_spatial: bool = False
    ecDNA_migration_delta_core: float = 0.0
    ecDNA_migration_delta_margin: float = 0.0
    ecDNA_migration_delta_infiltrating: float = 0.0
    ecDNA_migration_core_radius_um: float = 0.0
    ecDNA_migration_margin_radius_um: float = 0.0
