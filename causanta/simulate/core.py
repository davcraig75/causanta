...
class Simulation:
    ...
    def __init__(self, config_path: str | Path):
        ...
        # Output finalization policy. By default a finished run keeps the
        # summary HTMLs + animation (reports/), the small provenance files
        # (lineage.tsv, params.json, summary.log), and only the FINAL keyframe
        # of the per-timestep cells_t*/environment_t* snapshots; the empty
        # animations/ directory is removed. Set True to retain every snapshot
        # (needed for time-resolved re-analysis).
        self.keep_all_snapshots: bool = False
    ...
    def initialize(self) -> None:
        ...
        if self.config.output_dir:
            ...
            # Create subdirectories. The animation is written to reports/movie.html,
            # so no separate animations/ directory is created (it only ever ended
            # up empty); see _finalize_output for removal of any stale one.
    ...
    def run(self) -> None:
        ...
        if self.output_dir:
            ...
            # Finalize the output directory: the summary HTMLs, animation, and
            # results above are kept; the per-timestep snapshots (already
            # consumed by the movie/report) are trimmed to the final keyframe,
            # and any empty animations/ directory is removed.
            self._finalize_output(data_dir)

    def _finalize_output(self, data_dir: Path) -> None:
        """Keep reports + animation + final keyframe; drop the rest.

        Runs after every artifact (movie, viewer, report) has been generated,
        so it only changes on-disk persistence, never the simulation results.
        """
        if self.output_dir is None:
            return
        if not self.keep_all_snapshots:
            cleanup_data_keep_final_only(data_dir)
        else:
            print("  Keeping all per-timestep snapshots (--keep-all-snapshots)")
        # The animation lives in reports/movie.html; remove the empty animations/
        # directory if a stale one exists.
        anim = self.output_dir / "animations"
        if anim.is_dir() and not any(anim.iterdir()):
            anim.rmdir()
            print(f"  Removed empty {anim}/")
    ...
    def _step_cells(self) -> None:
        ...
        tumor_center = tumor_seed_centroid(self.config)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Retain every per-hour cells_t*/environment_t* TSV instead of "
            "trimming to the final keyframe. Use this when you need the full "
            "time series (e.g. time-resolved re-analysis). Default: keep only "
            "the final keyframe (the summary HTMLs, animation, and results are "
            "always kept)."
        ),
    )
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "After the run, gzip reports/movie.html in-place. "
            "Default: leave movie.html uncompressed."
        ),
    )
    ...
    # Snapshot trimming (keep only the final keyframe) and empty-animations
    # removal are handled inside sim.run()'s finalize step by default.
    sim.keep_all_snapshots = args.keep_all_snapshots

    try:
        sim.run()

        # Optional movie compression (opt-in). Runs after the movie has been
        # generated; affects only on-disk persistence, not the results.
        if sim.output_dir is not None and args.compress_movie:
