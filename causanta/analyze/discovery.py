...
def discover_causal_structure(
    ...
    **kwargs,
    ...
    if variables is None:
        # Use `is_hypoxic` (binary) rather than `O2_local` (continuous): the
        # simulator's EGFR and VEGF couplings to oxygen are mediated by the
        # HIF threshold (a step function at 18 mmHg), so the relevant signal
        # lives in the binary indicator. In late 6mm runs O2_local remains
        # highly variable but corr(O2_local, EGFR) ~ 0 because every tumor
        # cell is already above the threshold for the effect; switching to
        # is_hypoxic restores the structural link the PC algorithm needs.
        # Glucose is excluded because it tracks O2 via shared vascular
        # distance and causes multicollinearity.
        variables = [
            "ecDNA_count",
            "egfr_expression",
            "is_hypoxic",
