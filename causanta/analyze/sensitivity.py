...
def e_value_analysis(
    ...
    Args:
        estimate: Point estimate of causal effect (risk ratio scale)
    ...
    # For simplicity, assume estimate is already on appropriate scale
    # or is a standardized effect

    # E-value for point estimate
    if estimate <= 0:
        e_val = 1.0
    else:
        rr = np.exp(estimate) if abs(estimate) < 10 else (1 + estimate)
    ...
    if ci_lower <= 0:
        ...
        rr_ci = np.exp(ci_lower) if abs(ci_lower) < 10 else (1 + ci_lower)
        e_val_ci = rr_ci + np.sqrt(rr_ci * (rr_ci - 1))

    interpretation = (
        f"E-value = {e_val:.2f}. An unmeasured confounder would need to be "
        f"associated with both treatment and outcome by a factor of {e_val:.1f} "
        f"(risk ratio scale) to fully explain the observed effect. "
        f"E-value for CI lower bound = {e_val_ci:.2f}."
    )
